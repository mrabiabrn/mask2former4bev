...
def pixcoords3d(B, Y, X, N=6, device='cuda'):
    # returns a meshgrid sized B x Y x X
    grid_y = torch.linspace(0.0, Y-1, Y, device=torch.device(device))
    grid_y = torch.reshape(grid_y, [1, 1, Y, 1])
    grid_y = grid_y.repeat(B, N, 1, X)

    grid_x = torch.linspace(0.0, X-1, X, device=torch.device(device))
    grid_x = torch.reshape(grid_x, [1, 1, 1, X])
    grid_x = grid_x.repeat(B, N, Y, 1)
    
    grid_z = torch.ones(B, N, Y, X).to(device)

    # note we stack in xy order
    # (see https://pytorch.org/docs/stable/nn.functional.html#torch.nn.functional.grid_sample)
    grid = torch.stack([grid_x, grid_y, grid_z], dim=-1)
    grid = grid.permute(0,1,4,2,3).reshape(B,N,3,X*Y)
    return grid
