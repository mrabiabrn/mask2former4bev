...
class VizData(NuscData):
    ...
    def get_single_item(self, index, cams, refcam_id=None):
        ...
        lidar_data = np.transpose(lidar_data)       # n, 3
        ...
        lidar0_data = np.transpose(lidar0_data)
        lidar0_extra = np.transpose(lidar0_extra)
        lidar_data = np.transpose(lidar_data)       # 3, n
        lidar_extra = np.transpose(lidar_extra)
        ...
        seg_bev = hflip(seg_bev)
        valid_bev = hflip(valid_bev)


        if self.get_tids:
            return imgs, rots, trans, intrins, lidar0_data, lidar0_extra, lidar_data, lidar_extra, lrtlist, vislist, tidlist_, scorelist, seg_bev, valid_bev, center_bev, offset_bev, size_bev, ry_bev, ycoord_bev, radar_data, egopose, token
        else:
            return imgs, rots, trans, intrins, lidar0_data, lidar0_extra, lidar_data, lidar_extra, lrtlist, vislist, scorelist, seg_bev, valid_bev, center_bev, offset_bev, size_bev, ry_bev, ycoord_bev, radar_data, egopose, token
    ...
    def __getitem__(self, index):
        ...
        for index_t in self.indices[index]:
            ...
            imgs, rots, trans, intrins, lidar0_data, lidar0_extra, lidar_data, lidar_extra, lrtlist, vislist, tidlist, scorelist, seg_bev, valid_bev, center_bev, offset_bev, size_bev, ry_bev, ycoord_bev, radar_data, egopose, token = self.get_single_item(index_t, cams, refcam_id=refcam_id)
        ...
        #all_depth_data = torch.stack(all_depth_data)
        ...
        return all_imgs, all_rots, all_trans, all_intrins, all_lidar0_data, all_lidar0_extra, all_lidar_data, all_lidar_extra, all_lrtlist, all_vislist, all_tidlist, all_scorelist, all_seg_bev, all_valid_bev, all_center_bev, all_offset_bev, all_radar_data, all_egopose, all_tokens
...
# TODO: convert this function into a pytorch datamodule class. 
...
class NuscenesDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        nusc = NuScenes(version='v1.0-{}'.format(self.config['dataset_version']),
                    dataroot=os.path.join(self.config["dataset_path"]), #, version),
                    verbose=False)
        

        # take params from self.config

        # TODO: one timestep no temporal aug. modify/simplify code based on this.
        data_aug_conf = self.config["data_aug_conf"]
        centroid = self.config["centroid"]
        bounds = self.config["bounds"]
        res_3d = self.config["res_3d"]
        refcam_id = None #self.config["refcam_id"]
        get_tids = True #self.config["get_tids"]
        #temporal_aug = self.config["temporal_aug"]
        use_radar_filters = False #self.config["use_radar_filters"]
        do_shuffle_cams = False # True # self.config["do_shuffle_cams"]
    ...
    #     #sampler = torch.utils.data.DistributedSampler(self.val_dataset, shuffle=False)
